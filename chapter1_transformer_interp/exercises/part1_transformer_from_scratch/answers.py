...
class LayerNorm(nn.Module):
    ...
    def forward(self, residual: Float[Tensor, "batch posn d_model"]) -> Float[Tensor, "batch posn d_model"]:
        ...
        variance = t.var(residual, dim=-1, unbiased=False, keepdim=True)
        return (residual - mean) / ((variance + self.cfg.layer_norm_eps).sqrt()) * self.w + self.b
...
class Embed(nn.Module):
    ...
    def forward(self, tokens: Int[Tensor, "batch position"]) -> Float[Tensor, "batch position d_model"]:
        print("W_E", self.W_E.shape)
        print("1 token embedding", self.W_E[tokens].shape, self.W_E[5])
...
class Attention(nn.Module):
    ...
    def forward(self, normalized_resid_pre: Float[Tensor, "batch posn d_model"]) -> Float[Tensor, "batch posn d_model"]:
        Q: Float[Tensor, "batch posn n_heads d_head"] = (einops.einsum(normalized_resid_pre, self.W_Q, 'batch posn d_model, n_heads d_model d_head -> batch posn n_heads d_head') + self.b_Q)
        K: Float[Tensor, "batch posn n_heads d_head"] = (einops.einsum(normalized_resid_pre, self.W_K, 'batch posn d_model, n_heads d_model d_head -> batch posn n_heads d_head') + self.b_K)

        V: Float[Tensor, "batch posn n_heads d_head"] = (einops.einsum(normalized_resid_pre, self.W_V, 'batch posn d_model, n_heads d_model d_head -> batch posn n_heads d_head') +
        self.b_V)
        ...
        masked_attn = self.apply_causal_mask(attn_scores_scaled)
        attn_pattern = masked_attn.softmax(-1)

        z = einops.einsum(V, attn_pattern, "batch key_pos n_heads d_head, batch n_heads query_pos key_pos -> batch query_pos n_heads d_head")
...
class MLP(nn.Module):
    ...
    def forward(self, normalized_resid_mid: Float[Tensor, "batch posn d_model"]) -> Float[Tensor, "batch posn d_model"]:
        linear_in = einops.einsum(normalized_resid_mid, self.W_in, "batch posn d_model, d_model d_mlp -> batch posn d_mlp") + self.b_in
        gelu_val = gelu_new(linear_in)
        res = einops.einsum(gelu_val, self.W_out, "batch posn d_mlp, d_mlp d_model -> batch posn d_model") + self.b_out
        return res
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, resid_pre: Float[Tensor, "batch position d_model"]) -> Float[Tensor, "batch position d_model"]:
        attn_out_ln = self.attn(self.ln1(resid_pre)) + resid_pre
        mlp_out_ln = self.mlp(self.ln2(attn_out_ln)) + attn_out_ln
        return mlp_out_ln
...
class DemoTransformer(nn.Module):
    ...
    def forward(self, tokens: Int[Tensor, "batch position"]) -> Float[Tensor, "batch position d_vocab"]:
        ...
        res_out_ln = self.ln_final(residual)
        logits = self.unembed(res_out_ln)
        return logits
...
class TransformerTrainingArgs:
    ...
    lr = 1e-3
    weight_decay = 1e-2
    wandb_project: str | None = "demotransformer-test-2apr"
...
class TransformerTrainer:
    ...
    def training_step(self, batch: dict[str, Int[Tensor, "batch seq"]]) -> Float[Tensor, ""]:
        ...
        loss = -get_log_probs(tokens).mean()
        loss.backward()
        self.optimizer.step()
    ...
    def evaluate(self) -> float:
        ...
        for i, batch in tqdm(enumerate(self.test_loader), desc="Evaluating"):
            tokens = batch["tokens"].to(device)
...
class TransformerSampler:
    ...
    def sample(self, prompt: str, max_tokens_generated=100, verbose=False, **kwargs) -> str:
        ...
        for i in range(max_tokens_generated):
            ...
            token = t.tensor([TransformerSampler.sample_next_token(input_ids, logits_final, **kwargs)], device=device)
            input_ids = t.cat([input_ids, token], dim=-1)
            if verbose:
    ...
    def apply_frequency_penalty(
        ...
        """
        freq = t.bincount(input=input_ids, minlength=logits.size(0))
